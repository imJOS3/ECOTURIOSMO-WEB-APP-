...
import { CalendarIcon, RefreshIcon, SuccessIcon, ErrorIcon } from "../../../../components/common/icons/icons";
...
const TabReservasAnfitrion = () => {
  ...
  return (
    <div>
      {pendientes > 0 && (
        <div className="alert alert-amber" style={{ marginBottom: "1rem" }}>
          <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
            <CalendarIcon fontSize="small" /> Tienes <strong>{pendientes}</strong> reserva{pendientes !== 1 ? "s" : ""} pendiente{pendientes !== 1 ? "s" : ""} por confirmar
          </span>
        </div>
      )}
      ...
      {msg && (
        <div className={`alert alert-${msgType === "error" ? "error" : "success"}`} style={{ marginBottom: "1rem" }}>
          <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
            {msgType === "error" ? <ErrorIcon fontSize="small" /> : <SuccessIcon fontSize="small" />} {msg}
          </span>
        </div>
      )}
                ...
                <tr key={r.id}>
                  ...
                  <td>
                    <div className="mod-actions">
                      {r.estado === "pendiente" && (
                        <>
                          <button className="btn btn-sm btn-primary" onClick={() => cambiarEstado(r.id, "confirmada")}>
                            <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
                              <SuccessIcon fontSize="small" /> Confirmar
                            </span>
                          </button>
                          <button className="btn btn-sm btn-danger" onClick={() => cambiarEstado(r.id, "cancelada")}>
                            <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
                              <ErrorIcon fontSize="small" /> Cancelar
                            </span>
                          </button>
                        </>
                      )}
...
export default TabReservasAnfitrion;